...
void *ghostMovement(void *arg)
{
    ...
    if (!exitHome)
    {
        ...
        HousePermit.key = 1;
        HousePermit.permit = 1;
        if (!HousePermit.key && !HousePermit.permit)
        {
            hasKey = true;
            hasPermit = true;
            HousePermit.key = false;
            HousePermit.permit = false;
        }
    ...
    while (true)
    {
        ...
        if (!exitHome)
        {
            ...
            {
                exitHome = true; // Set exitHome to true once the ghost leaves the ghost house
            }
        }
        else
        {
            ...
            /* ghost->chdirection();
             ...
             // Adjust direction based on random selection
    ...
    while (window.isOpen())
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            {                         // Implement collision detection
                pacman.resetPacman(); // Reset Pac-Man if collision occurs
            }
        }
        sf::Event event;
        while (window.pollEvent(event))
